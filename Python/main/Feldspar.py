...
def Fd_Calc(data, data_index):
    ...
    sum_ca = SiO2_ca + Al2O3_ca + TiO2_ca + TFeO_ca + MnO_ca + MgO_ca + CaO_ca + K2O_ca + Na2O_ca
        
#---Electrovalency Difference Calculation of Fe2+/Fe3+---     
#General formula of pyroxene is M[T4O8], calculate the cation ratio on the basis of 5
    ca_ratio = sum_ca / 5
...
#Electrovalency of Cation ratio
    ...
    sum_el = SiO2_el + Al2O3_el + TiO2_el + TFeO_el + MnO_el + MgO_el + CaO_el + K2O_el + Na2O_el
#Ideal Anion Electrovalency = O * 8 = 16
#Calculation of Fe3+ and Fe2+
    ...
    sum_an = SiO2_ca * 2 + Al2O3_ca * 3 / 2 + TiO2_ca * 2 + FeO_wt / FeO_M * 1 + Fe2O3_wt / Fe2O3_M * 3 + MnO_ca * 1 + MgO_ca * 1 + CaO_ca * 1 + K2O_ca / 2  + Na2O_ca / 2
    
    an_ratio = sum_an / 8
#Normalizing the wt% and then calculate the anion ratio on the basis of 6O
    sum_wt = SiO2_wt + TiO2_wt + Al2O3_wt + Fe2O3_wt + FeO_wt + MnO_wt + MgO_wt + CaO_wt + K2O_wt + Na2O_wt
    ...
    Al6 = np.nan
    if float(SiO2_co2) < 4:
        if float(Al2O3_co2) < 4 - float(SiO2_co2):
            Al4 = float(Al2O3_co2)
            Al6 = 0 
        else:
    ...
    data_cali = {
                ...
                u'Or': float(K2O_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                u'Ab': float(Na2O_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                u'An': float(CaO_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                }
      
    return pd.DataFrame(data_cali, index = [data_index], columns = [u'SiO2', u'TiO2', u'Al2O3', u'Fe2O3', u'FeO', u'MnO', u'MgO', u'CaO', u'K2O', u'Na2O', u'Total', u'Comment', u'Si', u'Ti', u'Al', u'Al(4)', u'Al(6)', u'Fe3+', u'Fe2+', u'Mn', u'Mg', u'Ca', u'K', u'Na', u'Or', u'Ab', u'An'])
